...
docker exec jservice-db sh -c "PGPASSWORD=\$(cat \$POSTGRES_PASSWORD_FILE) pg_restore -U \$POSTGRES_USER -h \$POSTGRES_HOST -d \$POSTGRES_DB --clean --if-exists -F c \"$BACKUP_FILE\""
